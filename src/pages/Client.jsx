...
function Client() {
    ...
    const [hostConnected, setHostConnected] = useState(false)
    const [connectionQuality, setConnectionQuality] = useState("Unknown")
    const [localCursor, setLocalCursor] = useState({ x: -100, y: -100 }) // Offscreen initially
    const [remoteCursor, setRemoteCursor] = useState({ x: -100, y: -100 }) // Offscreen initially
    ...
    const updateConnectionQuality = () => {
        ...
        if (frameTimings.current.length > 10) {
            ...
            setConnectionQuality(
                avgFrameTime < 500 ? "Excellent" :
                    avgFrameTime < 1000 ? "Good" :
                        avgFrameTime < 2000 ? "Fair" : "Poor"
            )
        }
    }

    const acknowledgeFrame = (data) => {
        socket.emit("screen-data-received", {
            frameId: data.frameId,
            code: code,
            timestamp: Date.now(),
            ...
            <div className="bg-white p-4 rounded-lg shadow-lg">
                ...
                <div className="relative border-2 border-gray-300 rounded-md overflow-hidden">
                    ...
                    <div
                        ...
                        style={{
                            left: localCursor.x * (canvasRef.current?.offsetWidth / canvasSize.current.width),
                            top: localCursor.y * (canvasRef.current?.offsetHeight / canvasSize.current.height),
                ...
                <div className="mt-4 p-3 bg-gray-100 rounded-md text-sm">
                    <p>
                        Click on the screen to interact with the host computer. Press keys while the screen is focused to send
                        keyboard input. Cursor will remain visible during control.
                    </p>
                </div>
            </div>
        </div>
